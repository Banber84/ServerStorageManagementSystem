...
本文中的 IP、用户名和项目路径均为示例，实际部署以 `site.env`、
`/etc/ssms/nodes.conf` 和 `ssmsctl node list` 为准。
...
```bash
sudo ssmsctl node join NodeC 192.168.1.215 nodec1 --skip-existing-users
# 原脚本：sudo scripts/join_node.sh NodeC 192.168.1.215 nodec1 --skip-existing-users
```
